module.exports = async function handler(request, response) {
  ...
  const url = `${process.env.SUPABASE_URL}/rest/v1/gosa_results?id=eq.${encodeURIComponent(id)}&select=name,answers`;
  ...
  const row = rows[0];
  return response.status(200).json({ name: row.name, answers: row.answers });
};
